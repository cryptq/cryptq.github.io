...
SC_HAS_PROCESS(FFT_complex);

FFT_complex::FFT_complex(sc_module_name mName, const unsigned int c_uiFFTLength, const TranformType_t c_bInverse):
    sc_module(mName), 
...
void FFT_complex::ReadSamples()
{
    ...
    {     
        ...
        size_t iAvailable = complexIn.num_available();
        for( size_t i = 0; i < iAvailable; i++ ) // read all the values in order to empty the fifo
            complexIn.read();
...
void FFT_complex::OutputResults()
{
    ...
    {        
        ...
        {
            complexOut.write( std::complex<double>( 0,0 ) );     
        }
...
int FFT_complex::BitLength( unsigned int uiValue )
{
    int iCounter = 0;
...
sc_uint<32> FFT_complex::BitReverse( const unsigned int uiValue, const int iBitLength )
{
